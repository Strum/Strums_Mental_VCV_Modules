...
struct Knobs : Module {
  ...
  Knobs()
  {
    ...
    for (int i = 0; i < 3; ++i)
    {
      configParam(Knobs::KNOB_PARAM + i, -1.0, 1.0, 0.0, "");
      configParam(Knobs::SCALE_PARAM + i,0.0, 10.0, 1.0, "");
      configParam(Knobs::STEP_SWITCH + i, 0.0, 1.0, 0.0, "");
      configParam(Knobs::BI_SWITCH + i, 0.0, 1.0, 0.0, "");
      configParam(Knobs::STEPSIZE_SWITCH + i, 0.0, 1.0, 0.0, "");
    }
...
void Knobs::process(const ProcessArgs& args)
{
  for ( int i = 0 ; i < 3 ; i++)
  {
  // button triggers
...
//////////////////////////////////
struct KnobsWidget : ModuleWidget {
  KnobsWidget(Knobs *module)
{

  setModule(module);

  setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, "res/Knobs.svg")));

  int group_offset = 120;
  for (int i = 0 ; i < 3 ; i++)
  {
    addParam(createParam<MedKnob>(Vec(2, 20+group_offset*i), module, Knobs::KNOB_PARAM + i));
    addParam(createParam<MedKnob>(Vec(32, 20+group_offset*i), module, Knobs::SCALE_PARAM + i));

    addParam(createParam<LEDButton>(Vec(5, 50+group_offset*i), module, Knobs::STEP_SWITCH + i));
    addChild(createLight<MedLight<BlueLED>>(Vec(10, 55+group_offset*i), module, Knobs::BUTTON_LEDS + i ));
    addParam(createParam<LEDButton>(Vec(5, 75+group_offset*i), module, Knobs::BI_SWITCH + i));
    addChild(createLight<MedLight<BlueLED>>(Vec(10, 80+group_offset*i), module, Knobs::BUTTON_LEDS + 3 + i));
    addParam(createParam<LEDButton>(Vec(35, 50+group_offset*i), module, Knobs::STEPSIZE_SWITCH + i));
    addChild(createLight<MedLight<BlueLED>>(Vec(40, 55+group_offset*i), module, Knobs::BUTTON_LEDS + 6 + i));

    addOutput(createOutput<CVOutPort>(Vec(33, 75+group_offset*i), module, Knobs::OUTPUT + i));
  }
...
Model *modelKnobs = createModel<Knobs, KnobsWidget>("Knobs");
