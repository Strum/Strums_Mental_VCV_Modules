...
struct RadioButtons : Module
{
	...
	RadioButtons()
  {
    ...
    for (int i = 0; i < 7; ++i)
    {
      configParam(RadioButtons::BUTTON_PARAM +i, 0.0, 1.0, 0.0, "");
      configParam(RadioButtons::BUTTON2 + i, 0.0, 1.0, 0.0, "");
    }
  }
...
void RadioButtons::process(const ProcessArgs& args)
{
  for  (int i = 0 ; i < 7 ; i++)
  {
...
/////////////////////////////////////////////////////////////////////////
struct RadioButtonsWidget : ModuleWidget
{
  RadioButtonsWidget(RadioButtons *module)
  {
    setModule(module);

    setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, "res/RadioButtons.svg")));
    ...
    for (int i = 0; i < 7 ; i++)
    {
      addInput(createInput<GateInPort>(Vec(3, top_space + spacing * i), module, RadioButtons::INS1 + i));
      addOutput(createOutput<GateOutPort>(Vec(63, top_space + spacing * i), module, RadioButtons::OUTPUT + i));
      addParam(createParam<LEDButton>(Vec(35, top_space + 3 + spacing * i), module, RadioButtons::BUTTON_PARAM +i));
      addChild(createLight<MedLight<BlueLED>>(Vec(40, top_space + 8 + spacing * i), module, RadioButtons::BUTTON_LEDS + i));

  	  /// group 2
     addInput(createInput<GateInPort>(Vec(3, 10 + group_offset + spacing * i), module, RadioButtons::INS2 + i));
     addOutput(createOutput<GateOutPort>(Vec(63, 10 + group_offset +  spacing * i), module, RadioButtons::BUTTON2_OUT + i));
     addParam(createParam<LEDButton>(Vec(35, 10 + 3 + group_offset +  spacing * i), module, RadioButtons::BUTTON2 + i));
     addChild(createLight<MedLight<BlueLED>>(Vec(40,10 + 8 + group_offset +  spacing * i), module, RadioButtons::BUTTON2_LEDS + i));
    }
  }
};

Model *modelRadioButtons = createModel<RadioButtons, RadioButtonsWidget>("RadioButtons");
