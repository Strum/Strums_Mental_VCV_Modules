...
struct Cartesian : Module
{
	...
	Cartesian()
  {
    ...
    for (int i = 0; i < 4; ++i)
    {
      for (int j = 0; j < 4; ++j)
      {
        configParam(Cartesian::KNOB_PARAM + i + j * 4, -2.0, 2.0, 0.0, "");
        configParam(Cartesian::BUTTON_PARAM + i + j * 4, 0.0, 1.0, 0.0, "");
      }
    }
  }
...
void Cartesian::process(const ProcessArgs& args)
{
...
////////////////////////////////
struct CartesianWidget : ModuleWidget
{
  CartesianWidget(Cartesian *module)
  {

    setModule(module);

    setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, "res/Cartesian.svg")));
    ...
    int button_offset = 140;
  	addInput(createInput<GateInPort>(Vec(left, top), module, Cartesian::LEFT));
    addInput(createInput<GateInPort>(Vec(left+column_spacing, top), module, Cartesian::RIGHT));
    addInput(createInput<CVInPort>(Vec(left+column_spacing * 2, top), module, Cartesian::X_CV));
    addInput(createInput<GateInPort>(Vec(left+column_spacing * 3, top), module, Cartesian::X_RESET));

    addInput(createInput<GateInPort>(Vec(left, top + 30), module, Cartesian::UP));
    addInput(createInput<GateInPort>(Vec(left + column_spacing, top + 30), module, Cartesian::DOWN));
    addInput(createInput<CVInPort>(Vec(left + column_spacing * 2, top + 30), module, Cartesian::Y_CV));
    addInput(createInput<GateInPort>(Vec(left + column_spacing * 3, top + 30), module, Cartesian::Y_RESET));

    addInput(createInput<GateInPort>(Vec(left + column_spacing * 3, top + row_spacing * 2 + 10), module, Cartesian::RESET));

    addOutput(createOutput<CVOutPort>(Vec(213, 20), module, Cartesian::UNQUANT_OUT));
    addOutput(createOutput<CVOutPort>(Vec(213, 50), module, Cartesian::QUANT_OUT));

    for ( int i = 0 ; i < 4 ; i++)
    {
      for ( int j = 0 ; j < 4 ; j++)
      {
        addParam(createParam<SmlKnob>(Vec(left+column_spacing * i, top + row_spacing * j + 150 ), module, Cartesian::KNOB_PARAM + i + j * 4));
        addChild(createLight<MedLight<BlueLED>>(Vec(left+column_spacing * i + 4, top + row_spacing * j + 150 + 4), module, Cartesian::GRID_LIGHTS + i + j * 4));
        addParam(createParam<LEDButton>(Vec(button_offset+left+column_spacing * i, top + row_spacing * j + 150 ), module, Cartesian::BUTTON_PARAM + i + j * 4));
        addChild(createLight<MedLight<BlueLED>>(Vec(button_offset+left+column_spacing * i + 5, top + row_spacing * j + 150 + 5), module, Cartesian::BUTTON_LIGHTS + i + j * 4));
      }
      addOutput(createOutput<CVOutPort>(Vec(left+column_spacing * i, top + row_spacing * 4 + 150 ), module, Cartesian::ROW_OUT + i));
      addOutput(createOutput<CVOutPort>(Vec(left+column_spacing * 4, top + row_spacing * i + 150 ), module, Cartesian::COLUMN_OUT + i));
  	}
	}
};

Model *modelCartesian = createModel<Cartesian, CartesianWidget>("Cartesian");
