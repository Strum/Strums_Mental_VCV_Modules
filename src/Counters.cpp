...
Counters::Counters()
{
  ...
  configParam(Counters::COUNT_NUM_PARAM, 1.0, 32.0, 1.0, "");
  configParam(Counters::RST_BUTTON, 0.0, 1.0, 0.0, "");
  configParam(Counters::COUNT_NUM_PARAM_2, 1.0, 32.0, 1.0, "");
  configParam(Counters::RST_BUTTON_2, 0.0, 1.0, 0.0, "");
}

void Counters::process(const ProcessArgs& args)
{
...
struct CountersWidget : ModuleWidget
{
  CountersWidget(Counters *module)
  {
    setModule(module);

    setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, "res/Counters.svg")));

    int group_offset = 190;

    addParam(createParam<MedKnob>(Vec(2, 20), module, Counters::COUNT_NUM_PARAM));
    addInput(createInput<GateInPort>(Vec(3, 90), module, Counters::CLK_IN));
  	addInput(createInput<GateInPort>(Vec(3, 120), module, Counters::RESET_IN));

    addParam(createParam<LEDButton>(Vec(5, 145), module, Counters::RST_BUTTON));

    addOutput(createOutput<GateOutPort>(Vec(33, 90), module, Counters::OUTPUT));
    ...
    /////////// counter 2
    addParam(createParam<MedKnob>(Vec(2, 20 + group_offset), module, Counters::COUNT_NUM_PARAM_2));
    addInput(createInput<GateInPort>(Vec(3, 90 + group_offset), module, Counters::CLK_IN_2));
  	addInput(createInput<GateInPort>(Vec(3, 120 + group_offset), module, Counters::RESET_IN_2));

    addParam(createParam<LEDButton>(Vec(5, 145 + group_offset), module, Counters::RST_BUTTON_2));

    addOutput(createOutput<GateOutPort>(Vec(33, 90 + group_offset), module, Counters::OUTPUT_2));
...
Model *modelCounters = createModel<Counters, CountersWidget>("Counters");
